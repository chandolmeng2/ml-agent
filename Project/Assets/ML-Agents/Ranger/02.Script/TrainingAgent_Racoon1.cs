...
public class TrainingAgent_Racoon1 : Agent
{
    ...
    private void SpawnRandomObstacles()
    {
        ...
        while (spawnedObstacles.Count < obstacleCount && attempts < maxAttempts)
        {
            ...
            if (IsPositionOccupied(pos, 1.0f)) continue;

            GameObject prefabToSpawn = obstaclePrefabs[Random.Range(0, obstaclePrefabs.Length)];
            Quaternion randomRotation = Quaternion.Euler(0f, Random.Range(0f, 360f), 0f);

            GameObject obs = Instantiate(prefabToSpawn, pos + Vector3.up * 0f, randomRotation, transform.parent);
            obs.tag = "Obstacle";
            spawnedObstacles.Add(obs);
        }

        Debug.Log($"Spawned {spawnedObstacles.Count} obstacles in {attempts} attempts.");
    }
